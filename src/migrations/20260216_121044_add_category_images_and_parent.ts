...
export async function up({ db, payload, req }: MigrateUpArgs): Promise<void> {
  ...
  await db.run(sql`CREATE TABLE \`categories\` (
  	...
  	FOREIGN KEY (\`dropdown_image_id\`) REFERENCES \`media\`(\`id\`) ON UPDATE no action ON DELETE set null,
  	FOREIGN KEY (\`catalog_image_id\`) REFERENCES \`media\`(\`id\`) ON UPDATE no action ON DELETE set null
  );
  `)
  await db.run(sql`CREATE INDEX \`categories_parent_category_idx\` ON \`categories\` (\`parent_category_id\`);`)
  await db.run(sql`CREATE INDEX \`categories_dropdown_image_idx\` ON \`categories\` (\`dropdown_image_id\`);`)
  await db.run(sql`CREATE INDEX \`categories_catalog_image_idx\` ON \`categories\` (\`catalog_image_id\`);`)
  ...
  await db.run(sql`CREATE TABLE \`products\` (
  	...
  	\`model\` text,
  	\`title\` text,
  	\`description_mode\` text DEFAULT 'visual',
  	\`description\` text,
  	...
  	\`price_in_u_s_d_enabled\` integer,
  	\`price_in_u_s_d\` numeric,
  	\`specifications_title\` text DEFAULT 'Характеристики',
  	\`specifications_view\` text DEFAULT 'grouped',
  	\`meta_title\` text,
  	\`meta_image_id\` integer,
  	\`meta_description\` text,
  	\`generate_slug\` integer DEFAULT true,
  ...
  await db.run(sql`CREATE TABLE \`_products_v\` (
  	...
  	\`version_model\` text,
  	\`version_title\` text,
  	\`version_description_mode\` text DEFAULT 'visual',
  	\`version_description\` text,
  	\`version_extra_description\` text,
  	\`version_description_h_t_m_l\` text,
  	\`version_inventory\` numeric DEFAULT 0,
  	\`version_enable_variants\` integer,
  	\`version_price_in_u_s_d_enabled\` integer,
  	\`version_price_in_u_s_d\` numeric,
  	\`version_specifications_title\` text DEFAULT 'Характеристики',
  	\`version_specifications_view\` text DEFAULT 'grouped',
  	\`version_meta_title\` text,
  	\`version_meta_image_id\` integer,
  	\`version_meta_description\` text,
  	\`version_generate_slug\` integer DEFAULT true,
